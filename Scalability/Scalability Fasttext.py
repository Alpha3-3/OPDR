...
if __name__ == '__main__':
    ...
    # --- SCALABILITY TEST CONFIGURATION for Fasttext ---
    ...
    # Fixed parameters for this series of runs (Fasttext)
    fixed_alpha = 6
    fixed_b = 90
    ...
    dummy_feature_dim_fasttext = fixed_original_dimension

    for f_name in training_files_to_test:
        if not os.path.exists(f_name):
            print(f"Creating dummy file: {f_name} with {dummy_feature_dim_fasttext} features.")
            try:
                num_samples = int(f_name.split('_')[2]) # Extracts '300', '600', etc.
                dummy_data = np.random.rand(num_samples, dummy_feature_dim_fasttext).astype(np.float32)
                np.save(f_name, dummy_data)
            except Exception as e:
                print(f"Could not create dummy file {f_name}: {e}")

    if not os.path.exists(test_file_global):
        print(f"Creating dummy file: {test_file_global} with {dummy_feature_dim_fasttext} features.")
        try:
            # Assuming test set size is 300 for dummy creation if not specified otherwise
            dummy_data_test = np.random.rand(300, dummy_feature_dim_fasttext).astype(np.float32)
