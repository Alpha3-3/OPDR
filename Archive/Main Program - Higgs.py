...
# Parameter settings
b_values = [45,50,55]  # Only used if DW-PMAD is enabled
k_values = [1,3, 6, 10, 15]
alpha_values = [40,45,50,55]  # Only used if DW-PMAD is enabled
...
if __name__ == '__main__':
    ...
    test_results_df.to_csv('parameter_sweep_results_Higgs_Multiple_methods_Batch.csv', index=False)
    print(test_results_df)
    print("Test results exported to 'parameter_sweep_results_Higgs_Multiple_methods_Batch.csv'")

    # Save training (A) results.
    training_results_df = pd.DataFrame(list(training_results_list), columns=columns)
    training_results_df.to_csv('training_accuracy_results_Higgs_Multiple_methods.csv', index=False)
    print(training_results_df)
    print("Training results exported to 'training_accuracy_results_Higgs_Multiple_methods.csv'")
